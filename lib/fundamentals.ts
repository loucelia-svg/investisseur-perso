...
> {
  ...
  console.log(
    ...
    `📚 Börsenlotse ${company} ${statement}: ${url}`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        ...
        `⚠️ Börsenlotse ${company} ${statement}: HTTP ${response.status}`
    ...
    console.log(
      ...
      `📄 Börsenlotse ${company} ${statement}: ${html.length} caractères`
...
async function fetchBoersenlotseFcfPerShare(
  ...
  const symbol = BOERSENLOTSE_SYMBOLS[company];

  const url =
    `${BOERSENLOTSE_BASE}/${symbol}/finanzen` +
    `?ansicht=je-aktie&statement=cashflow&zeitraum=alle`;

  console.log(`📚 Börsenlotse ${company} FCF/action: ${url}`);
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `⚠️ Börsenlotse ${company} FCF/action: HTTP ${response.status}`
      );
      return new Map();
    }
    ...
    console.error(`❌ Börsenlotse ${company} FCF/action:`, error);
    return new Map();
  }
}
...
export async function getYahooHistoricalFundamentals(
   ...
   */
  const START_YEAR = currentYear - 6;
  const CHUNK_YEARS = 4;
...
async function getCompanyHistoricalFundamentals(
  ...
  const [
    ...
    fetchBoersenlotseStatement(company, "guv"),
    fetchBoersenlotseStatement(company, "balance"),
    fetchBoersenlotseStatement(company, "cashflow"),
    fetchBoersenlotseStatement(company, "kennzahlen"),
    fetchBoersenlotseFcfPerShare(company),
  ]);
  ...
  const boersenlotseRows =
    ...
    Array.from(years)
      ...
      .map((year) => {
         ...
         * totalDebt est TOUJOURS recalculé à partir

         * de la dette financière CT + LT lorsqu'au

         * moins l'une des deux existe.

         *

         * On ne remplace pas cela par "Verbindlichkeiten gesamt",

         * qui représente l'ensemble des passifs et non la dette

         * financière.

         */
        /**
         * Historique des actions diluées.
         *
         * Si dilutedShares manque, on le retrouve avec :
         * Free Cash Flow / Free Cash Flow par action.
         * Une donnée absente reste null.
         */
        if (
  ...
  const yahooRows =
    ...
    await getYahooHistoricalFundamentals(
