    ...
    setLiveStocks(
      stocks.map((stock) => {
        const liveStock = data.find(
          (item: { isin: string; tradingDateTime?: string; price?: number }) =>
...
const alertsTriggered = liveStocks.flatMap((stock) =>
  ...
  currency: "EUR",
                  ...
                  <p className="mt-1 text-sm text-slate-500">
  Cours Euronext
  {"tradingDateTime" in stock && stock.tradingDateTime
    ? ` — ${new Date(stock.tradingDateTime).toLocaleString("fr-FR", {
        dateStyle: "short",
        timeStyle: "short",
      })}`
    : ""}
  ...
  <section className="mb-10 rounded-2xl border border-red-200 bg-red-50 p-6">
    ...
    <div className="mt-3 space-y-2">
      {alertsTriggered.map((item) => (
        <p key={`${item.stock}-${item.alert}`} className="text-red-700">
          🚨 {item.stock} ≤{" "}
